...
![Overfitting Demonstration]({{ site.baseurl }}/img/chapter_img/chapter07/overfitting_demonstration.png)
...
![Regularization Priors]({{ site.baseurl }}/img/chapter_img/chapter07/regularization_priors.png)
...
### 4.3. Checklist chon prior strength trong ML workflow

Trong thuc te machine learning, prior strength (hoac regularization strength) nen duoc chon theo mot quy trinh ro rang thay vi thu-nghiem ngau nhien:

1. **Define prediction target + metric**: xac dinh metric phu hop bai toan (RMSE, MAE, log-loss, calibration...).
2. **Split data ngay tu dau**: tach train/validation/test truoc khi tune bat ky tham so nao.
3. **Fit preprocessing tren train only**: standardization, imputation, encoding phai hoc tren train va apply sang validation/test de tranh leakage.
4. **Tune prior scale tren validation**: quet mot luoi prior scales hop ly va so sanh performance ngoai mau.
5. **Kiem tra prior predictive**: loai bo prior sinh ra du doan vo ly truoc khi nhin ket qua fit chi tiet.
6. **Chot model tren test mot lan**: chi danh gia test o buoc cuoi cung de bao toan tinh khach quan.

Checklist tren ket noi truc tiep Bayes voi quy trinh huan luyen ML: prior khong chi la phan "ly thuyet", ma la mot hyperparameter co y nghia mo hinh hoa.

## 5. Applied Case Study (Phan 1): Setup, split, va leakage control

Ta dung mot bai toan regression high-dimensional gia lap: du doan gia tri lien tuc tu 25 predictors, trong do chi mot so predictor that su co tin hieu. Muc tieu cua Phan 1 la xay dung pipeline du lieu dung cach.
...
# Sparse ground truth: chi 6 bien co signal
beta_true = np.zeros(p)
beta_true[:6] = np.array([2.4, -1.8, 1.2, 0.9, -0.7, 0.5])

X = rng.normal(0, 1, size=(n, p))
y = 1.5 + X @ beta_true + rng.normal(0, 1.2, size=n)
...
# Preprocessing fit tren train only (anti-leakage)
...
- Neu du lieu da split va scale dung quy trinh train-only, ban moi nen bat dau tune prior/regularization.
- Neu preprocessing duoc fit tren full data truoc split, ket qua validation se bi lac quan gia tao do leakage.

## 6. Khi nào Cần Regularization?
...
Bai tiep theo: **Bias-Variance Tradeoff** (di sau vao validation strategy va model-complexity tuning trong cung case study).
