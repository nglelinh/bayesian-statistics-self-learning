...
Sau bài này, bạn cần hiểu interaction effect (hiệu ứng tương tác) là gì và vì sao nhiều hiện tượng thực tế không thể được mô tả bằng mô hình cộng đơn giản. Bạn cũng cần biết cách đọc interaction theo kiểu "ảnh hưởng của một biến thay đổi theo bối cảnh của biến khác" thay vì cố ép mọi coefficient (hệ số) thành các hiệu ứng cố định.
...
Mô hình này nói rằng ảnh hưởng của $$x_1$$ là như nhau ở mọi mức của $$x_2$$, và ảnh hưởng của $$x_2$$ cũng là như nhau ở mọi mức của $$x_1$$. Nhưng trong thực tế, rất nhiều hiện tượng không vận hành như vậy: hiệu quả của quảng cáo phụ thuộc vào chất lượng sản phẩm, tác động của chế độ ăn phụ thuộc vào mức độ vận động, ảnh hưởng của số giờ học phụ thuộc vào nền tảng kiến thức, và tác động của thuốc phụ thuộc vào độ tuổi hoặc tình trạng bệnh nền.

Khi đó ta cần interaction.

## 2. Interaction effect là gì?

Interaction xảy ra khi ảnh hưởng của một predictor (biến dự báo) lên outcome (biến kết quả) **thay đổi theo giá trị của predictor khác**.
...
Trong mô hình additive (mô hình cộng), các đường gần như song song. Điều đó có nghĩa là khác nhóm thì mức nền có thể khác, nhưng tác động của $$x_1$$ lên $$y$$ gần như giống nhau giữa các nhóm.

### Mô hình có interaction

Trong mô hình có interaction, các đường có slope khác nhau rõ rệt. Điều đó có nghĩa là cùng tăng một đơn vị $$x_1$$, nhưng outcome tăng mạnh hay yếu còn tùy vào mức của $$x_2$$.
...
Outcome ở đây là điểm cuối kỳ, còn predictors là số giờ học mỗi tuần và trình độ đầu vào.
...
Khi đó, ảnh hưởng của giờ học phụ thuộc vào trình độ đầu vào, nên interaction là hợp lý.
...
Nói cách khác, effect (hiệu ứng) của ngân sách quảng cáo phụ thuộc vào chất lượng sản phẩm.

### Ví dụ 3: điều trị và mức độ bệnh

Một phác đồ có thể rất hiệu quả với ca nhẹ nhưng ít hiệu quả hơn với ca rất nặng, hoặc ngược lại. Khi đó effect của treatment thay đổi theo severity (mức độ nặng của bệnh).
...
Nếu $$x_2$$ là biến nhóm, trong đó $$x_2 = 0$$ là nhóm đối chứng còn $$x_2 = 1$$ là nhóm can thiệp, thì ở nhóm đối chứng, effect của $$x_1$$ là $$\beta_1$$, còn ở nhóm can thiệp, effect của $$x_1$$ là $$\beta_1 + \beta_3$$.
...
Nếu hiểu biết ngành nói rằng effect thay đổi theo bối cảnh, interaction nên được xem xét. Những ví dụ quen thuộc là thuốc và tuổi, đào tạo và trình độ nền, hay giá và phân khúc khách hàng.
...
Không hẳn. Main effect (hiệu ứng chính) vẫn có nghĩa, nhưng nghĩa của nó phụ thuộc vào cách bạn mã hóa và center variables (đưa biến về quanh trung tâm). Nếu biến liên tục được center quanh giá trị trung bình, thì main effect thường là ảnh hưởng của predictor tại mức trung bình của biến kia.
...
Chưa đủ. Với interaction, hình vẽ và conditional effects (hiệu ứng có điều kiện) quan trọng ngang hoặc hơn con số tóm tắt.

## 9. Một ví dụ gần gũi: tập luyện và chế độ ăn

Giả sử outcome là số kg giảm trong 8 tuần, còn predictors là số buổi tập mỗi tuần và mức độ tuân thủ chế độ ăn.
...
Đến cuối chapter này, bạn đã có một bộ công cụ rất quan trọng để đọc hồi quy cho đúng, bao gồm nhiều predictors cùng lúc, confounding và causal structure (cấu trúc nhân quả), multicollinearity, và interaction effects. Đây là lúc hồi quy thôi không còn là "vẽ một đường thẳng", mà trở thành một cách suy nghĩ về cơ chế tạo dữ liệu.

> **3 ý cần nhớ.** Interaction xảy ra khi ảnh hưởng của một predictor thay đổi theo mức của predictor khác, nên không thể luôn diễn giải hiệu ứng như một con số cố định cho mọi trường hợp; khi mô hình có interaction, cách đọc tốt nhất là đọc theo conditional effects và dùng hình minh họa thay vì chỉ nhìn bảng coefficients; và interaction rất thường gặp trong đời sống thật vì hiệu quả của can thiệp, hành vi hay điều kiện hiếm khi giống nhau trong mọi bối cảnh.
