...
Bài học này trình bày **rất chi tiết** về hàm mật độ xác suất (PDF), sự khác biệt với hàm khối xác suất (PMF), và cách hiểu đúng về PDF trong bối cảnh biến ngẫu nhiên liên tục. Sau khi học xong bài này, bạn sẽ hiểu tại sao PDF không phải là xác suất, tại sao $$f(x)$$ có thể lớn hơn 1, và cách sử dụng PDF đúng cách trong phân tích Bayesian.
...
**Tính chất của PMF**:
1. $$P(X = x) \geq 0$$ với mọi $$x$$ (không âm)
2. $$\sum_{\text{all } x} P(X = x) = 1$$ (tổng bằng 1)
3. $$P(X = x)$$ **là xác suất thực sự** với ý nghĩa trực tiếp
...
![PMF vs Biến Liên tục](/bayesian-statistics-self-learning/img/chapter_img/chapter00/pmf_vs_continuous.png)
...
**Chú ý quan trọng nhất**: 
- $$f(x)$$ **KHÔNG PHẢI** là xác suất!
- $$f(x)$$ là **mật độ** xác suất
- Chỉ có **tích phân** (diện tích) của $$f(x)$$ mới là xác suất
...
Tương tự trong thống kê:
- Mật độ xác suất $$f(x)$$ không phải là xác suất
- $$f(x)$$ cho biết xác suất "tập trung" như thế nào quanh $$x$$
- Xác suất thực tế = $$\int f(x) \, dx$$ (diện tích dưới đường cong)

![Tương tự Mật độ](/bayesian-statistics-self-learning/img/chapter_img/chapter00/density_analogy.png)
...
![PDF có thể > 1](/bayesian-statistics-self-learning/img/chapter_img/chapter00/pdf_can_exceed_one.png)
...
![Histogram tiến đến PDF](/bayesian-statistics-self-learning/img/chapter_img/chapter00/histogram_to_pdf.png)
...
![Xấp xỉ Khoảng Nhỏ](/bayesian-statistics-self-learning/img/chapter_img/chapter00/small_interval_approximation.png)
...
Một khía cạnh quan trọng nhưng thường bị bỏ qua của PDF là đơn vị của nó. Nếu biến ngẫu nhiên $$X$$ có đơn vị (ví dụ: cm, kg, giây), thì:

- $$f(x)$$ có đơn vị: **1/(đơn vị của X)**
- $$f(x) \cdot dx$$ có đơn vị: **(1/(đơn vị của X)) × (đơn vị của X) = không đơn vị** (xác suất)
...
![Đơn vị của PDF](/bayesian-statistics-self-learning/img/chapter_img/chapter00/pdf_units.png)
...
![So sánh PMF và PDF](/bayesian-statistics-self-learning/img/chapter_img/chapter00/pmf_pdf_comparison.png)
...
Đây là sai lầm phổ biến nhất. Nhiều người nhìn vào phân phối chuẩn với trung bình 170 cm và độ lệch chuẩn 10 cm, tính được $$f(170) = 0.04$$, rồi nói "xác suất chiều cao bằng 170 cm là 0.04". Điều này hoàn toàn **SAI**.

- ❌ **SAI**: "$$P(X = 170) = f(170) = 0.04$$"
- ✓ **ĐÚNG**: "$$P(X = 170) = 0$$ (biến liên tục), $$f(170) = 0.04$$ là mật độ"

### Sai lầm 2: Nghĩ $$f(x) \leq 1$$

Vì xác suất không bao giờ vượt quá 1, nhiều người tự động giả định rằng PDF cũng không thể vượt quá 1. Nhưng như chúng ta đã thấy, điều này **SAI**. PDF có thể và thường xuyên lớn hơn 1.

- ❌ **SAI**: "$$f(x)$$ phải nhỏ hơn hoặc bằng 1"
- ✓ **ĐÚNG**: "$$f(x) \geq 0$$ và có thể > 1, miễn $$\int f(x)dx = 1$$"
...
Vì $$f(x)$$ phụ thuộc vào đơn vị, việc so sánh $$f(170 \text{ cm})$$ với $$f(1.70 \text{ m})$$ là vô nghĩa. Chúng có đơn vị khác nhau (1/cm vs 1/m) và giá trị số học khác nhau.

- ❌ **SAI**: "So sánh $$f(170 \text{ cm})$$ với $$f(1.70 \text{ m})$$"
- ✓ **ĐÚNG**: "Chỉ so sánh xác suất (không đơn vị)"

## 6. Ứng dụng trong Bayesian Statistics

### 6.1. Prior và Posterior là PDF

Trong phân tích Bayesian, cả phân phối prior $$p(\theta)$$ và phân phối posterior $$p(\theta \mid x)$$ đều là PDF (nếu tham số $$\theta$$ liên tục). Điều này có nghĩa là:

- $$p(\theta)$$ không phải là xác suất mà là **mật độ tin cậy** (density of belief)
- $$p(\theta = 0.5)$$ không có ý nghĩa; chỉ có $$P(a \leq \theta \leq b) = \int_a^b p(\theta) d\theta$$ có ý nghĩa
- $$p(\theta)$$ có thể lớn hơn 1

![Ứng dụng Bayesian](/bayesian-statistics-self-learning/img/chapter_img/chapter00/bayesian_pdf_application.png)
...
![Tóm tắt PDF](/bayesian-statistics-self-learning/img/chapter_img/chapter00/pdf_summary_infographic.png)
...
1. **$$f(x)$$ là mật độ, KHÔNG phải xác suất** - Đây là điểm cốt lõi nhất. Giống như mật độ khối lượng trong vật lý không phải là khối lượng.

2. **Chỉ có tích phân của $$f(x)$$ mới là xác suất** - $$P(a \leq X \leq b) = \int_a^b f(x)dx$$. Diện tích dưới đường cong là xác suất.

3. **$$f(x)$$ có thể lớn hơn 1** - Vì không phải xác suất, $$f(x)$$ không bị giới hạn bởi 1.

4. **$$P(X = x) = 0$$ với biến liên tục** - Xác suất tại một điểm chính xác luôn bằng 0.

5. **$$f(x)$$ có đơn vị 1/(đơn vị của X)** - PDF thay đổi theo hệ đơn vị, nhưng xác suất không đổi.

6. **PDF là giới hạn của histogram** - Khi $$n \to \infty$$ và bin width $$\to 0$$, histogram tiến đến PDF.

7. **Trong Bayesian, prior và posterior là PDF** - Chúng ta làm việc với mật độ tin cậy, không phải xác suất điểm.
