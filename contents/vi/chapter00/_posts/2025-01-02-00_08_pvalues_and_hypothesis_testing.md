---
layout: post
title: "Bài 0.8: P-values và Kiểm định Giả thuyết"
chapter: '00'
order: 8
...
- **Giả thuyết không** ($$H_0$$): Thuốc không có hiệu quả (sự thay đổi trung bình trong huyết áp là 0)
- **Giả thuyết thay thế** ($$H_1$$): Thuốc có hiệu quả (sự thay đổi trung bình trong huyết áp khác 0)
...
- Ta **giả sử** $$H_0$$ đúng
- Sau đó hỏi: 👉 *"Nếu $$H_0$$ đúng, thì dữ liệu như thế này có hiếm không?"*

**Ví dụ:**

- $$H_0$$: thuốc không có tác dụng
- Dữ liệu cho thấy thuốc có vẻ hiệu quả
- **P-value trả lời:** Nếu thuốc thật sự không có tác dụng, thì kết quả như thế này xảy ra với xác suất bao nhiêu?

#### (2) "Dữ liệu bất thường" nghĩa là gì?

"Bất thường" không phải là cảm tính, mà được định nghĩa bằng:

- **Thống kê kiểm định** (z-score, t-statistic, $$\chi^2$$, …)
- **Khoảng cách** so với kỳ vọng theo $$H_0$$

**Ví dụ:**

- Trung bình mẫu lệch xa trung bình theo $$H_0$$
- Sai khác lớn giữa hai nhóm
- Giá trị thống kê nằm ở đuôi phân phối

→ Càng "xa", càng "cực đoan" → càng "bất thường"
...
- Quan sát: $$z = 2.1$$
- P-value = $$P(\mid Z \mid \geq 2.1 \mid H_0 \text{ đúng})$$
...
Đây là các hiểu sai rất phổ biến:

❌ **P-value ≠ xác suất giả thuyết không là sai**

❌ **P-value ≠ xác suất kết quả là do ngẫu nhiên**

❌ **P-value nhỏ ≠ hiệu ứng lớn**
...
- **P nhỏ** → dữ liệu khó xảy ra nếu $$H_0$$ đúng → nghi ngờ $$H_0$$
- **P lớn** → dữ liệu phù hợp với $$H_0$$
---
...
Chúng ta thiết lập:
- $$H_0$$: $$\mu = 170$$ (chiều cao trung bình quần thể là 170 cm)
- $$H_1$$: $$\mu \neq 170$$ (chiều cao trung bình quần thể khác 170 cm)
...
Lưu ý những gì p-value **không** cho chúng ta biết:
- Nó không cho biết xác suất $$H_0$$ đúng là 5.4%
- Nó không cho biết xác suất $$H_1$$ đúng là 94.6%
- Nó không cho biết kích thước hiệu ứng (effect size) là bao nhiêu
- Nó không cho biết nghiên cứu có ý nghĩa thực tiễn hay không
...
- Nếu $$p < \alpha$$: Bác bỏ $$H_0$$, kết luận kết quả là "có ý nghĩa thống kê"
- Nếu $$p \geq \alpha$$: Không bác bỏ $$H_0$$, kết luận kết quả "không có ý nghĩa thống kê"
...
Việc dựa vào ngưỡng cứng nhắc này có nhiều hậu quả tiêu cực:
- Nó khuyến khích "p-hacking" (thao túng phân tích để đạt được $$p < 0.05$$)
- Nó dẫn đến tư duy nhị phân (có/không có hiệu ứng) thay vì tư duy về mức độ bằng chứng
- Nó bỏ qua thông tin quan trọng về kích thước hiệu ứng và độ không chắc chắn
...
Với tất cả những vấn đề này, tại sao p-values và kiểm định giả thuyết vẫn chiếm ưu thế trong nghiên cứu khoa học? Có một số lý do:

1. **Lịch sử và quán tính**: P-values đã được dạy và sử dụng trong nhiều thập kỷ. Thay đổi thực hành thống kê trong toàn bộ cộng đồng khoa học là một quá trình chậm.

2. **Tính đơn giản bề ngoài**: P-values cung cấp một con số duy nhất và một quy tắc quyết định đơn giản. Điều này hấp dẫn, mặc dù tính đơn giản này là ảo tưởng.

3. **Yêu cầu xuất bản**: Nhiều tạp chí khoa học truyền thống yêu cầu hoặc ưu tiên các nghiên cứu với kết quả "có ý nghĩa thống kê" ($$p < 0.05$$), tạo ra một hệ thống khuyến khích sai lệch.

4. **Thiếu đào tạo về phương pháp thay thế**: Nhiều nhà nghiên cứu không được đào tạo về phương pháp Bayesian hoặc các phương pháp thống kê hiện đại khác.
...
Hơn nữa, phương pháp Bayesian cho phép chúng ta:
- Tính xác suất của các giả thuyết khác nhau
- Kết hợp thông tin từ nhiều nghiên cứu một cách tự nhiên
- Đưa ra dự đoán về dữ liệu tương lai
- Định lượng độ không chắc chắn một cách trực tiếp và dễ diễn giải

## Khi nào Nên Sử dụng P-values?

Mặc dù có nhiều hạn chế, p-values không phải lúc nào cũng sai. Có một số tình huống mà chúng có thể hữu ích:

1. **Phát hiện bất thường**: P-values có thể là một công cụ sàng lọc nhanh để phát hiện các mẫu trong dữ liệu đáng được điều tra thêm.

2. **Kiểm tra mô hình**: P-values có thể được sử dụng để kiểm tra xem một mô hình có phù hợp với dữ liệu hay không (posterior predictive checks trong Bayesian cũng làm điều này tốt hơn).

3. **Khi quyết định nhị phân là cần thiết**: Trong một số tình huống (như kiểm soát chất lượng), chúng ta thực sự cần đưa ra quyết định nhị phân, và kiểm định giả thuyết có thể phù hợp.
...
Nếu bạn thấy mình cần sử dụng hoặc diễn giải p-values, hãy tuân theo những hướng dẫn này:

1. **Không bao giờ chỉ báo cáo p-value**: Luôn báo cáo kích thước hiệu ứng, khoảng tin cậy, và kích thước mẫu.

2. **Không sử dụng ngưỡng cứng nhắc**: Tránh tư duy nhị phân "có ý nghĩa/không có ý nghĩa". Xem p-value như một thước đo liên tục của mức độ bất thường của dữ liệu.

3. **Diễn giải cẩn thận**: Luôn nhớ rằng p-value là $$P(\text{dữ liệu} \mid H_0)$$, không phải $$P(H_0 \mid \text{dữ liệu})$$.

4. **Xem xét bối cảnh**: Một p-value nhỏ không tự động có nghĩa là kết quả quan trọng. Xem xét kích thước hiệu ứng, ý nghĩa thực tiễn, và chất lượng nghiên cứu.

5. **Cân nhắc phương pháp thay thế**: Đối với nhiều câu hỏi nghiên cứu, phương pháp Bayesian hoặc các phương pháp dựa trên ước lượng (estimation-based methods) sẽ phù hợp hơn.
...
Khi bạn tiến xa hơn trong khóa học này, bạn sẽ thấy rằng phương pháp Bayesian giải quyết nhiều vấn đề mà chúng ta đã thảo luận:

- Nó cho phép chúng ta nói về xác suất của giả thuyết và tham số
- Nó cung cấp các ước lượng trực tiếp và dễ diễn giải của độ không chắc chắn
- Nó không dựa vào ngưỡng tùy ý hoặc quyết định nhị phân
- Nó cho phép chúng ta kết hợp kiến thức trước đó một cách tự nhiên
- Nó tập trung vào ước lượng và dự đoán, không chỉ kiểm định
